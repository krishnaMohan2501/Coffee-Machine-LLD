...
public class HotCoffee extends Beverages {
    ...
    public boolean canServe(Map<String, Integer> allIngredientWithValues) {
        ...
        if (this.getHotWater() > allIngredientWithValues.get(IngredientEnum.HOT_WATER.getIngredient())) {
            isServable = false;
            resultList.getResults().add(BeverageType.HOT_COFFEE.getBeverageType() + " " + "cannot be prepared because " + IngredientEnum.HOT_WATER.getIngredient() +
                    "is not " +
                    "sufficient");

        } else if (this.getGingerSyrup() > allIngredientWithValues.get(IngredientEnum.GINGER_SYRUP.getIngredient())) {
            isServable = false;
            resultList.getResults().add(BeverageType.HOT_COFFEE.getBeverageType() + " " + "cannot be prepared because " + IngredientEnum.GINGER_SYRUP.getIngredient() +
                    "is not " +
                    "sufficient");

        } else if (this.getSugarSyrup() > allIngredientWithValues.get(IngredientEnum.SUGAR_SYRUP.getIngredient())) {
            isServable = false;
            resultList.getResults().add(BeverageType.HOT_COFFEE.getBeverageType() + " " + "cannot be prepared because " + IngredientEnum.SUGAR_SYRUP.getIngredient() +
                    "is not " +
                    "sufficient");

        } else if (this.getTeaLeavesSyrup() > allIngredientWithValues.get(IngredientEnum.TEA_LEAVES_SYRUP.getIngredient())) {
            isServable = false;
            resultList.getResults().add(BeverageType.HOT_COFFEE.getBeverageType() + " " + "cannot be prepared because " + IngredientEnum.TEA_LEAVES_SYRUP.getIngredient() +
                    "is not " +
                    "sufficient");
        } else if (this.getTeaLeavesSyrup() > allIngredientWithValues.get(IngredientEnum.HOT_MILK.getIngredient())) {
            isServable = false;
            resultList.getResults().add(BeverageType.HOT_COFFEE.getBeverageType() + " " + "cannot be prepared because " + IngredientEnum.HOT_MILK.getIngredient() +
                    "is not " +
                    "sufficient");
        }
        return isServable;
    ...
    public boolean isAllIngredientPresent(Map<String, Integer> allIngredientWithValues) {
        ...
        if(allIngredientWithValues.get(IngredientEnum.HOT_WATER.getIngredient()) == -1){
            ...
            allPresent = false;
            System.out.println(BeverageType.HOT_COFFEE.getBeverageType() + " " + "cannot be prepared because " + IngredientEnum.SUGAR_SYRUP.getIngredient() +
                    "is not " +
                    "available");
